...
import { Save, GripVertical, AlertCircle, Download, Upload } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  const handleReconcileConfirm = () => {
    ...
    saveItems(updatedItems) // Save the modified inventory items
  ...
  const exportConfiguration = () => {
    const config = {
      ...
      generalSettings: getSettings('GENERAL_SETTINGS'),
  ...
  const importConfiguration = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Import all settings
        saveSettings('CATEGORIES', config.categories);
        saveSettings('UNITS', config.units);
        saveSettings('LOCATIONS', config.locations);
        saveSettings('SUPPLIERS', config.suppliers);
        saveSettings('PROJECTS', config.projects);
        if (config.generalSettings) {
          saveSettings('GENERAL_SETTINGS', config.generalSettings);
        }
